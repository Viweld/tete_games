  ...
  for (final Frame frame in trace.frames) {
    if (frame is UnparsedFrame) {
      if (_obfuscatedStackTraceLineRegExp.hasMatch(frame.member)) {
        ...
        elements.add(<String, String>{'file': '', 'line': '0', 'method': frame.member});
      }
    } else {
...
String? getBuildId(StackTrace stackTrace) {
  ...
  for (final Frame frame in trace.frames) {
    if (frame is UnparsedFrame) {
      if (frame.member.startsWith("build_id: '") && frame.member.endsWith("'")) {
...
List<String> getLoadingUnits(StackTrace stackTrace) => Trace.parseVM(stackTrace.toString())
    .terse
    .frames
