...
Future<void> mainCommon(Flavor flavor) async {
  ...
  final String envFileName = switch (flavor) {
    Flavor.prod => '.env.prod',
    Flavor.dev || Flavor.devMock => '.env.dev',
  };

  try {
    await dotenv.load(fileName: envFileName);
  } on Object catch (error) {
    log('Could not load $envFileName: $error');
    rethrow;
  }

  final AppRuntimeMode runtimeMode = await AppRuntimeModeStorage.read();

  await configureDependencies(flavor: flavor, runtimeMode: runtimeMode);
