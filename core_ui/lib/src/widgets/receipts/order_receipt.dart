...
class OrderReceipt {
  ...
  OrderReceipt._({required this.order, required this.styles, required this.logoSvg});

  static Future<Uint8List> generate(BuildContext extContext, OrderDraft order) async {
    ...
    final OrderReceipt widget = OrderReceipt._(order: order, styles: styles, logoSvg: logoSvg);
  ...
  pw.Widget build(pw.Context pwContext, BuildContext extContext) {
    ...
    final String receiptDate = createdAt == null
        ? ''
        : DateFormat('d MMMM yyyy', localeName).format(createdAt);
            ...
            children: <pw.Widget>[
              pw.Column(
                ...
                children: <pw.Widget>[
                  ...
                  pw.Text(
                    localization.receipt_pdf_date_external(receiptDate, order.id),
                    style: styles.b2,
                  ),
                  pw.SizedBox(height: 8 * s),
                  pw.Text(localization.receipt_pdf_order_number(order.refNumber), style: styles.b2),
                ],
              ),
              pw.SizedBox(
                width: 220 * s,
                                        ...
                                        child: ReceiptSectionCard(
                                          ...
                                          child: pw.Column(
                                            ...
                                            children: <pw.Widget>[
                                              ...
                                              if (transfer.stopsList.isNotEmpty)
                                                ...transfer.stopsList.map(
                                                  (OrderTransferStop sl) => pw.Padding(
                                                    padding: const pw.EdgeInsets.only(top: 20 * s),
                                                    child: ReceiptInfoField(
                                                      label:
                                                          localization.receipt_transfer_stop_point,
                                                      value: sl.address ?? '',
                                                      styles: styles,
                                                    ),
                                                  ),
                                              ...
                                              ReceiptInfoField(
                                                label: localization.transfer_arrival_address_label,
                ...
                pw.Expanded(
                  child: pw.Column(
                    ...
                    children: <pw.Widget>[
                      ...
                      if (order.conditions.isNotEmpty) ...<pw.Widget>[
                        ReceiptSectionCard(
                            ...
                            pw.Column(
                              ...
                              children: <pw.Widget>[
                                ReceiptKeyValueRow(
                                  ...
                                  value: _paymentStatusText(order.status, localization),
                                  styles: styles,
                                ),
                                if (createdAt != null)
                                  ReceiptKeyValueRow(
                                    label: localization.receipt_label_date_colon,
                                    value: DateFormat('dd.MM.yyyy', localeName).format(createdAt),
                                    styles: styles,
                                  ),
                                ReceiptKeyValueRow(
  ...
  String _paymentStatusText(OrderStatus status, AppLocalization localization) => switch (status) {
    OrderStatus.waiting_payment => localization.receipt_payment_status_awaiting,
    OrderStatus.paid => localization.receipt_payment_status_paid,
    OrderStatus.cancelled => localization.receipt_payment_status_declined,
    _ => localization.receipt_payment_status_unknown_long,
  };
}
